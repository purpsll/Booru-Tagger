...
- Stash with external-plugin support. The workflow was exercised live on Stash v0.31.1 during development, and the v3.26.11 release workflow runs the full automated regression suite before packaging the public release.
...
For images already marked `Multi-Booru Unresolved` by the normal Fast Scan, v3.26.11 reuses that completed fast-stage result instead of repeating exact MD5 and local pHash lookups. Forced rechecks such as Review and No-Match retries still reevaluate the full lookup path.
...
The UI validates that the chosen URL is still the active Review candidate before either action is applied. v3.26.11 also corrects the `ImageDetailPanel` `patch.after` callback signature for Stash v0.31.1 so React's second `{}` context argument is not mistaken for the rendered component.
