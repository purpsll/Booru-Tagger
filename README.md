...
The same source also publishes **Stash Metadata Migrator v1.1**, a separate migration add-on for restoring an old Stash JSON export onto media already scanned into a new Stash. It matches media by exact MD5/OShash fingerprints so changed paths do not create a second set of Scene/Image/File records; safely reconciles Tags, Performers, and Studios; and restores Galleries, Groups, chapters, Scene markers, hierarchy, artwork, custom fields, and history. Restore automatically creates a Stash database backup before changing metadata.
